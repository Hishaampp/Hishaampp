...
def build_stats_svg(total, weeks):
    W, H = 480, 150
    out = [svg_header(W, H)]
    out.append(label(20, 34, "contributions · last 12 months", size=11, fill=DIM))
    out.append(label(20, 72, f"{total:,}", size=34, weight=700))

    # weekly sparkline — continuity is defensible at the weekly aggregate
    if weeks:
        chart_x, chart_y, chart_w, chart_h = 20, 95, W - 40, 40
        mx = max(weeks) or 1
        n = len(weeks)
        pts = []
...
def main():
    ...
    outputs = {
        "stats.svg": build_stats_svg(total, weeks),
